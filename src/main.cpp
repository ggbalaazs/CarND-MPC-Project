      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // transform waypoints from global coords to car coords
          ...
          // 3rd order polynomial fitting on waypoints
          auto coeffs = polyfit(x_waypoints, y_waypoints, 3);
          // 3rd order polynomial fitting on waypoints x coords
          for (uint i = 0; i < ptsx.size(); ++i) {
            next_x_vals.emplace_back(x_waypoints[i]);
            next_y_vals.emplace_back(polyeval(coeffs, x_waypoints[i]));
          }

          // calculate cross-track error and orientation error
          double cte = polyeval(coeffs, 0);
          double epsi = -atan(coeffs[1]);     // no further tags, x0 is 0

          // px, py and psi are normally 0 in car coords
          ...
          uint N = int(result.size() / 2 - 1);
          for (uint i = 0; i < N; ++i) {
            mpc_x_vals.emplace_back(result[i * 2 + 2]);
            mpc_y_vals.emplace_back(result[i * 2 + 3]);
          }
